...
class SGobject:
    def __init__(self):
        ...
        # Use a private variable for the actual storage
        self._point_assignment_mode = "Nearest"
    ...
    def point_assignment_mode(self, value):
        if value in ["Nearest", "NoOverlap", "All"]:
            ...
            raise ValueError(
                "point_assignment_mode must be 'Nearest', 'NoOverlap', or 'All'.")
    ...
    def mask_to_objects(self, tiff_path, tolerance=1.5, object_column_name='nucleus'):
        ...
        for geom, value in mask_polygons:
            simplified_shape = shape(geom).simplify(
                tolerance, preserve_topology=True)
            features.append(simplified_shape)
            # Could probably avoid the cast to int32
            object_id.append(str(np.int32(value)))

        self.gdf = gpd.GeoDataFrame(
            {'object_id': object_id, object_column_name: features}, geometry=object_column_name)

    def nimbus_json_to_objects(self, json_path, object_column_name='nucleus', filter_tags=[]):
        ...
        for annotation in data['annotations']:
            if annotation['shape'] == 'polygon':
                if len(filter_tags) > 0:
                    if any(tag in annotation['tags'] for tag in filter_tags):
                        # Extract coordinates for each polygon and create a Shapely Polygon object
                        polygon = Polygon([(coord['x'], coord['y'])
    ...
    def dilate_objects(self, radius=10.0, identifier=None, output_name=None, set_geometry=True):
        ...
        if identifier is None:
            # Use the current geometry column name if identifier is None
            identifier = self.gdf.geometry.name
        ...
        print(
            f"Dilation completed. Output column: '{output_name}'{' is now the active geometry column.' if set_geometry else '.'}")
    ...
    def load_points(self, csv_path):
        ...
        df = pd.read_csv(csv_path, usecols=[
                         'name', 'refid', 'x', 'y'], dtype={'refid': str})
    ...
    def create_cell_gene_table(self, index_col='object_id', point_assignment_mode=None):
        ...
        if point_assignment_mode is not None:
            # This will use the setter and its validation
            self.point_assignment_mode = point_assignment_mode
        else:
        ...
        self.assigned_points_gdf = gpd.sjoin(
            self.points_gdf, self.gdf, how='left', predicate='within')
        ...
        self.assigned_points_gdf['distance_to_edge'] = self.assigned_points_gdf.apply(
            lambda row: distance_to_polygon_edge(
                ...
                self.gdf.geometry.loc[row['index_right']] if not pd.isna(
                    row['index_right']) else None
            ),
            axis=1
        )
        ...
        if self.point_assignment_mode == "Nearest":
            ...
            self.assigned_points_gdf = self.assigned_points_gdf.sort_values(
                by="distance_to_edge", ascending=False)
            self.assigned_points_gdf.drop_duplicates(
                subset=["name", "geometry"], keep="first", inplace=True)
            ...
            print(
                "Error: Invalid point_assignment_mode. Please choose from 'Nearest', 'NoOverlap', or 'All'.")
            return

        # Continue with your existing logic using self.assigned_points_gdf
        total_points = len(self.assigned_points_gdf)
        # Count non-null entries for the specified index column
        assigned_points = self.assigned_points_gdf[index_col].notnull().sum()
    ...
    def run_proseg(self, output_path='output.csv', regenerate_proseg_polygons=False):
        ...
        if self.points_gdf is None or self.assigned_points_gdf is None:
            raise ValueError(
                "Points not loaded/assigned. Please load data and assign points before exporting.")

        # Merge GeoDataFrames
        merged_gdf = self.points_gdf.merge(self.assigned_points_gdf[[
                                           'object_id']], left_index=True, right_index=True, how='left')
        ...
        if not os.path.exists(geojson_path) or regenerate_proseg_polygons:
            ...
            try:
                result = subprocess.run(
                    proseg_command, check=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if not os.path.exists(geojson_path):
                ...
                try:
                    subprocess.run(gunzip_command, check=True, text=True,
    ...
    def _load_and_process_geometries(self, geojson_path):
        ...
        geo_df['geometry'] = geo_df['geometry'].apply(lambda geom: scale(
        ...
        for geom in geo_df.geometry:
            if geom.geom_type == 'MultiPolygon':
                ...
                simplified_polygon = largest_polygon.simplify(
                    1.43, preserve_topology=True)
                processed_geometries.append(simplified_polygon)
            else:
                count_polygon += 1
                simplified_polygon = geom.simplify(
                    1.43, preserve_topology=True)
                processed_geometries.append(simplified_polygon)

        # Replace original with simplified
        geo_df['geometry'] = processed_geometries
        # Ensure 'geometry' is still the active geometry column
        geo_df.set_geometry('geometry', inplace=True)

        print(
            f"Total MultiPolygons processed into largest Polygons: {count_multipolygon}")
        ...
        # Assign processed geometries to new 'cell' column
        geo_df['cell'] = processed_geometries
    ...
    def add_filter(self, filter_mask, filter_column="default_filter"):
        ...
        if len(filter_mask) != self.cell_gene_table.n_obs:
            print(
                "Error: Filter mask length must match the number of observations in the AnnData object.")
            return
    ...
    def remove_filters(self, filter_name=None):
        ...
        if filter_name is None:
            ...
            filter_columns = [
                col for col in self.cell_gene_table.obs.columns if self.cell_gene_table.obs[col].dtype == 'bool']
            ...
            if filter_name in self.cell_gene_table.obs.columns:
                self.cell_gene_table.obs.drop(
                    columns=[filter_name], inplace=True)
                print(f"Filter '{filter_name}' removed from obs DataFrame.")
            else:
                print(
                    f"Error: Filter '{filter_name}' not found in obs DataFrame.")

    # Make a function to return the cell_gene_table as a DataFrame

    def get_cell_gene_table_df(self, **kwargs):
        # Default to True if use_filter is not provided
        ...
        if use_filter:
            if filter_column in self.cell_gene_table.obs:
                ...
                print(
                    f"*** NOTE: {filter_column} being applied, using {num_filtered} of {num_total} total objects ***")
            else:
                if not filter_column in self.cell_gene_table.obs:
                    # TODO: Add a quiet mode to decide whether to print these sorts of messages
                    # print("No filtering available or applied.")
                    pass
                else:
                    print(
                        f"Filter column '{filter_column}' not applied. Returning unfiltered data.")
        return df
    ...
    def plot_gene_scatter(self, gene1, gene2, **kwargs):
        ...
        if gene1 not in cell_gene_table_df.columns or gene2 not in cell_gene_table_df.columns:
            print(
                f"One or both genes ({gene1}, {gene2}) not found in cell_gene_table.")
            return

        # Extract occurrences for each gene
        gene1_counts = cell_gene_table_df.get(
            gene1, 0)  # Default to 0 if gene not found
        gene2_counts = cell_gene_table_df.get(gene2, 0)

        # Plot scatter
        plt.figure(figsize=(8, 6))
        plt.scatter(gene1_counts, gene2_counts, alpha=0.6,
                    edgecolors='w', color='blue')
    ...
    def show_gene_stats_plots(self, gene_name=None, **kwargs):
        ...
        if gene_name in cell_gene_table_df.columns:
            ...
            cell_gene_table_df[gene_name].plot(
                kind='hist', bins=100, title=f'Gene expression distribution for {gene_name}')
            plt.show()

            # Plot the gene expression levels on the polygons
            merged_gdf = self.gdf.merge(
                cell_gene_table_df, on='object_id', how='left')
            fig, ax = plt.subplots(figsize=(10, 10))
            merged_gdf.plot(column=gene_name, cmap='Wistia',
                            legend=True, ax=ax)
            ax.set_aspect('equal')
            plt.show()
            ...
            print(
                f"Number of objects expressing {gene_name}: {num_expressing} out of {len(cell_gene_table_df)} ({num_expressing / len(cell_gene_table_df) * 100:.2f}%)")
        else:
            print(
                f"Error: Gene '{gene_name}' not found in the cell count matrix.")
    ...
    def generate_statistics_and_histograms(self):
        ...
        if self.gdf.empty:
            print(
                "GeoDataFrame (gdf) is missing. Please run appropriate loading function.")
            return
        if self.points_gdf.empty:
            print(
                "Points GeoDataFrame (points_gdf) is missing. Please run appropriate loading function.")
            return
        ...
        if self.cell_gene_table is None:
            print(
                "Cell Count DataFrame (cell_gene_table) is missing. Please run appropriate loading function.")
            return
        ...
        assigned_points = self.assigned_points_gdf.dropna(
            subset=['index_right'])  # Assuming 'index_right' is used for join
        percent_assigned = 100 * \
            len(assigned_points) / num_points if num_points else 0
        avg_assigned_points_per_object = len(
            assigned_points) / num_objects if num_objects else 0

        # Unique genes detected per object
        genes_per_object = assigned_points.groupby('index_right')[
            'name'].nunique()
        ...
        print(
            f"Average number of points per object: {avg_points_per_object:.2f}")
        print(
            f"Percentage of points assigned to an object: {percent_assigned:.2f}%")
        print(
            f"Average number of assigned points per object: {avg_assigned_points_per_object:.2f}")
        print(
            f"Average number of unique genes detected per object: {avg_genes_per_object:.2f}")
        print(f"Total number of genes detected: {total_genes}")
        print(
            f"Total number of genes detected (assigned to objects): {total_assigned_genes}")

        # Histograms
        plt.figure(figsize=(10, 4))
        plt.subplot(1, 2, 1)
        plt.hist(genes_per_object, bins=np.arange(genes_per_object.min(),
                 genes_per_object.max() + 1), color='skyblue', edgecolor='black')
        ...
        plt.hist(points_per_object, bins=np.arange(points_per_object.min(
        ), points_per_object.max() + 1), color='lightgreen', edgecolor='black')
    ...
    def plot_polygon_and_points(self, identifier, id_field='object_id', gene_names=None, annotate=True):
        ...
        other_polygons = self.gdf[self.gdf.geometry.intersects(
            expanded_bbox) & (self.gdf[id_field] != identifier)]

        if gene_names is not None:
            if isinstance(gene_names, str):
                gene_names = [gene_names]
            # first pick the subset of points corresponding to the gene names
            points_within_bbox = self.assigned_points_gdf[self.assigned_points_gdf['name'].isin(
                gene_names)]
            # then filter to those within the expanded bbox
            points_within_bbox = points_within_bbox[points_within_bbox.geometry.within(
                expanded_bbox)]
        else:
            # if no gene names are passed, get all the ones in the expanded bbox
            points_within_bbox = self.assigned_points_gdf[self.assigned_points_gdf.geometry.within(
                expanded_bbox)]
        ...
        for name, group in points_within_bbox.groupby('name'):
            ...
            if not interior_points.empty:
                ax.scatter(interior_points.geometry.x, interior_points.geometry.y,
                           marker='o', s=50, edgecolor='black', color=color_map[name])

            # Plot exterior points with 'x' marker style
            if not exterior_points.empty:
                ax.scatter(exterior_points.geometry.x, exterior_points.geometry.y,
                           marker='x', s=50, color=color_map[name])
    ...
    def l_metric(self, gene1, gene2, plot_hexbin=False, plot_gene_counts=False, plot_cumulative_sum=False, **kwargs):
        ...
        if plot_gene_counts:
            ...
            plt.scatter(
                x_values, df_sorted[f"random_{gene2}"], color='green', label="Random", s=10, alpha=0.5)
            plt.scatter(
                x_values, df_sorted[gene1], color='blue', label=gene1, s=10, alpha=0.5)
            plt.scatter(
                x_values, df_sorted[gene2], color='red', label=gene2, s=10, alpha=0.5)
        ...
        df_sorted['cumsum_random_' +
                  gene2] = df_sorted[f"random_{gene2}"].cumsum()

        if plot_cumulative_sum:
            plt.figure(figsize=(10, 6))
            plt.plot(df_sorted['cumsum_' + gene1], df_sorted['cumsum_' +
                     gene2], color='red', label=f'{gene1} vs. {gene2}')
            plt.plot(df_sorted['cumsum_' + gene1], df_sorted['cumsum_random_' +
                     gene2], color='green', label=f'{gene1} vs. Random {gene2}')
        ...
        for i in range(len(df_sorted) - 1):
            deltaX = df_sorted['cumsum_' + gene1].iloc[i +
                                                       1] - df_sorted['cumsum_' + gene1].iloc[i]
            deltaY = ((df_sorted['cumsum_' + gene2].iloc[i+1] - df_sorted['cumsum_random_' + gene2].iloc[i+1]) + (
                df_sorted['cumsum_' + gene2].iloc[i] - df_sorted['cumsum_random_' + gene2].iloc[i])) / 2
            area_difference += deltaX * deltaY
            total_area += deltaX * \
                (df_sorted['cumsum_' + gene2].iloc[i] +
                 df_sorted['cumsum_' + gene2].iloc[i+1])/2

        area_difference_normalized = area_difference / \
            (df_sorted['cumsum_' + gene1].iloc[-1]
             * df_sorted['cumsum_' + gene2].iloc[-1])
        total_area_normalized = total_area / \
            (df_sorted['cumsum_' + gene1].iloc[-1]
             * df_sorted['cumsum_' + gene2].iloc[-1])
    ...
    def export_to_nimbus_json(self, output_file_path, object_name=None, gene_names=None, datasetId="unknown",
        ...
        if obs_variable and obs_variable not in self.cell_gene_table.obs.columns:
            raise ValueError(
                f"The specified obs_variable '{obs_variable}' does not exist in the obs DataFrame.")
        ...
        if obs_variable:
            if self.cell_gene_table.obs[obs_variable].dtype.name in ['object', 'category']:
                ...
                color_mapping = {cat: "#{:06x}".format(
                    random.randint(0, 0xFFFFFF)) for cat in unique_categories}
            else:
        ...
        for index, row in self.gdf.iterrows():
            ...
            if obs_variable:
                try:
                    ...
                    skipped_objects += 1
                    warnings.warn(
                        f"Object ID {row['object_id']} not found in cell_gene_table.obs. Skipping obs_variable for this object.")
            ...
            for index, row in self.gdf.iterrows():
                ...
                try:
                    if object_id in cell_gene_df.index:
                        gene_counts = cell_gene_df.loc[object_id,
                                                       genes_to_export]
                        counts_dict = {gene: int(count)
                    ...
                    skipped_objects += 1
                    warnings.warn(
                        f"Object ID {object_id} not found in cell_gene_table. Skipping gene counts for this object.")

        # Export points
        if gene_names:
            filtered_points_gdf = self.points_gdf[self.points_gdf['name'].isin(
                gene_names)]
        else:
        ...
        if len(filtered_points_gdf) > 500000:
            raise ValueError(
                "There are more than 500,000 points. Please specify gene_names to filter.")
        ...
        for index, row in filtered_points_gdf.iterrows():
            ...
            if randomize_spot_colors:
                point['color'] = color_mapping.get(
                    row['name'], "#000000")  # Default color if not found
        ...
        if include_connections:
            ...
            for index, row in self.assigned_points_gdf.iterrows():
                # Check if the point is assigned to a nucleus
                if pd.notnull(row['object_id']):
                    connection = {
                        ...
                        # The point ID
                        "childId": point_id_mapping.get(index, None),
                        "datasetId": datasetId
                    }
                    # Only add connection if the point was included
                    if connection["childId"] is not None:
        ...
        if skipped_objects > 0:
            print(
                f"Warning: {skipped_objects} objects were skipped due to missing data")
